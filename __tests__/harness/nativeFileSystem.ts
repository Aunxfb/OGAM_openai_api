// Buffer is global in the Jest (node) runtime, so the harness uses it directly
// instead of the `buffer` npm polyfill (not a dependency on this branch).
const { Buffer } = globalThis as unknown as typeof import('buffer');
