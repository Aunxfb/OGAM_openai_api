...
import { manageContextWindow, dropMissingImageAttachments } from './llmMessages';
...
class LLMService {
  ...
  async generateResponse(messages: Message[], options?: { onStream?: StreamCallback; onComplete?: CompleteCallback; disableThinking?: boolean }): Promise<string> {
    ...
    const completionWork = (async () => {
      const managed = await dropMissingImageAttachments(await manageContextWindow(messages));
  ...
  async getContextDebugInfo(messages: Message[]) {
    const managed = await manageContextWindow(messages);
